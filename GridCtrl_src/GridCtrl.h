...
// 网格线或者滚动条的选择
#define GVL_NONE                0L      // 无网格线
#define GVL_HORZ                1L      // 仅有水平网格线
#define GVL_VERT                2L      // 仅有垂直网格线
#define GVL_BOTH                3L      // 水平和垂直网格线都有

// 自动调整大小选项
#define GVS_DEFAULT             0		// 默认
#define GVS_HEADER              1       // 仅用于列的固定单元格数据
#define GVS_DATA                2       // 仅用于列的非固定单元格数据
#define GVS_BOTH                3       // 固定列和非固定列都适用

// 单元格查找选项
#define GVNI_FOCUSED            0x0001	// 查找焦点单元格
#define GVNI_SELECTED           0x0002	// 查找已选单元格
#define GVNI_DROPHILITED        0x0004	// 查找 Drop highlighted单元格
#define GVNI_READONLY           0x0008	// 查找只读单元格
#define GVNI_FIXED              0x0010	// 查找锁定单元格
#define GVNI_MODIFIED           0x0020	// 查找修改过的单元格

#define GVNI_ABOVE              LVNI_ABOVE	// 在初始单元格上方查找
#define GVNI_BELOW              LVNI_BELOW	// 在初始单元格下文查找
#define GVNI_TOLEFT             LVNI_TOLEFT	// 向左查找初始单元格
#define GVNI_TORIGHT            LVNI_TORIGHT// 向右查找初始单元格
#define GVNI_ALL                (LVNI_BELOW|LVNI_TORIGHT|LVNI_TOLEFT) // 从指定单元格开始查找全部单元格
#define GVNI_AREA               (LVNI_BELOW|LVNI_TORIGHT)	// 从指定单元格右下方查找单元格
...
// 通知消息。当不使用 NM_GRIDVIEW 结构体时，这些消息就像它们的 LVN_... 副本
#define GVN_BEGINDRAG           LVN_BEGINDRAG       // 拖曳发生时发送 LVN_FIRST-9
#define GVN_BEGINLABELEDIT      LVN_BEGINLABELEDIT  // 定位编辑开始时发送 LVN_FIRST-5
#define GVN_ENDLABELEDIT        LVN_ENDLABELEDIT    // 定位编辑停止时发送 LVN_FIRST-6
#define GVN_BEGINRDRAG          LVN_BEGINRDRAG		// 
#define GVN_COLUMNCLICK         LVN_COLUMNCLICK
#define GVN_DELETEITEM          LVN_DELETEITEM
#define GVN_SELCHANGING         LVN_ITEMCHANGING	// 单元格选择改变之前发送
#define GVN_SELCHANGED          LVN_ITEMCHANGED		// 单元格选择改变之后发送
#define GVN_GETDISPINFO         LVN_GETDISPINFO		// 当 Grid 处在虚拟模式下的一个单元格信息请求
#define GVN_ODCACHEHINT         LVN_ODCACHEHINT		// 虚拟模式下的隐藏提示
...
/* 大小和位置函数 */
	...
	// 获取 Cell 的左上点、范围、所包含文本的范围
    BOOL GetCellOrigin(int nRow, int nCol, LPPOINT p);	// Cell 的左上点
    BOOL GetCellOrigin(const CCellID& cell, LPPOINT p);	
    BOOL GetCellRect(int nRow, int nCol, LPRECT pRect);	// Cell 的范围
    BOOL GetCellRect(const CCellID& cell, LPRECT pRect);
    BOOL GetTextRect(const CCellID& cell, LPRECT pRect);// Cell 内文本的范围
...
/* 总体的外观和特征 */
	// 设置和获取 Grid 的当前图像列表（指针操作）
    void SetImageList(CImageList* pList)          { m_pImageList = pList;             }
    CImageList* GetImageList() const              { return m_pImageList;              }
	// 哪些线条不可见
    void SetGridLines(int nWhichLines = GVL_BOTH);// Both vertical and horizontal lines are displayed.
	...
	// 隐藏的行/列（高/宽为0）在用户调整行/列大小时是否可以显示出来
