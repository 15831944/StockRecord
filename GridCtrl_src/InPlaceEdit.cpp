                           ...
                           UINT nFirstChar)
{
    ...
    m_nColumn       = nColumn;
    m_nLastChar     = 0; 
	// 若是通过点击鼠标产生编辑的框，则移动方向健（在内容间移动）不会退出。
	// m_bExitOnArrows 为静态变量，会影响 OnKeyDown() 。
...
// 方向键（或相关的）按下，则在以下情况时退出当前编辑窗口。
// If an arrow key (or associated) is pressed, then exit if
//  a) The Ctrl key was down, or
//  b) m_bExitOnArrows == TRUE
void CInPlaceEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
    if ( (nChar == VK_PRIOR || nChar == VK_NEXT || nChar == VK_DOWN  || nChar == VK_UP ||
...
{
    if (nChar == VK_TAB || nChar == VK_RETURN)	// 确认对单元格的编辑。
    {
        m_nLastChar = nChar;
        GetParent()->SetFocus();	// This will destroy this window
        return;
    }
    if (nChar == VK_ESCAPE)						// 放弃对单元格的编辑，恢复前值。
    {
