...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
    ...
    TRY
    {
        m_arRowHeights.SetSize(m_nRows);    // 设置行/列的数量
    ...
    int i;
    for (i = 0; i < m_nRows; i++)
        m_arRowHeights[i] = m_cellDefault.GetHeight();	// 初始化各行高
    for (i = 0; i < m_nCols; i++)
        m_arColWidths[i] = m_cellDefault.GetWidth();	// 初始化各列宽
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    if (IsCTRLpressed())
    {
        switch (nChar)								// CTRL + A/X/C/V	全选/剪切/复制/粘贴
        {
        case 'A':
            OnEditSelectAll();
            break;
...
#ifndef GRIDCONTROL_NO_CLIPBOARD
    if (IsSHIFTpressed() &&(nChar == VK_INSERT))	// 插入: SHIFT+Insert --> Paste
        OnEditPaste();
#endif
    ...
    if (nChar == VK_DELETE)							// 删除: Delete --> Cut
    {
		CutSelectedText();
    }
    else if (nChar == VK_DOWN)						// 下键: Down
    {
    ...
    else if (nChar == VK_NEXT)						// PageDown
    {
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    m_LeftClickDownCell  = GetCellFromPt(point);// 得到左击时的单元格位置。
    if (!IsValid(m_LeftClickDownCell))			// 确保单元格可见。
        return;
    ...
    if ((nFlags & MK_SHIFT) != MK_SHIFT)		// 没有按下 SHIFT 键。
        m_SelectionStartCell = m_LeftClickDownCell;
    else										// 按下 SHIFT 键，选择的是一个区域。
    {
    ...
    if (pCell)
    {
        ...
        {
            bInTextArea = rectCell.PtInRect(point);	// 左击时的点是否在文本区域中。
        }
    }

    // If the user clicks on the current cell, then prepare to edit it.
    // (If the user moves the mouse, then dragging occurs)
    if (m_LeftClickDownCell == m_idCurrentCell &&	// 两次单击，则可能会编辑。
        !(nFlags & MK_CONTROL) && bInTextArea &&	// 没有按下CONTROL ，确保在文本区域中。
        IsCellEditable(m_LeftClickDownCell))		// 确保单元格是可编辑的。
    {
